...
public class RozpocznijObsluge extends BasicSimEvent<Smo, Zgloszenie>
{
    ...
    public RozpocznijObsluge(Smo parent,int numer) throws SimControlException
    {
    	super(parent);
    	generator = new SimGenerator();
        this.smoParent = parent;
	...
	protected void stateChange() throws SimControlException {
        if (smoParent.liczbaZgl() > 0)
        {
            ...
            System.out.println(simTime()+" - "+simDate(SimDateField.HOUR24)+" - "+simDate(SimDateField.MINUTE)+" - "+simDate(SimDateField.SECOND)+" - "+simDate(SimDateField.MILLISECOND)+": SMO- (Kanał: "+this.numerObslugi+")Początek obsługi zgl. nr: " + zgl.getTenNr());
        	
        	// Zaplanuj koniec obsługi
        	//smoParent.zakonczObsluge = new ZakonczObsluge(smoParent, czasObslugi, zgl);      
            smoParent.zakonczObsluge = new ZakonczObsluge(smoParent, czasObslugi, zgl,this.numerObslugi);
        }
